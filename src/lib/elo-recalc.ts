...
export interface EloRecalcResult {
  tournamentMatches: number;
  leagueResults: number;
  eloUpdates: number;
}

export async function recalculateElo(db: D1Database): Promise<EloRecalcResult> {
  const players = (
    await db.prepare("SELECT id, ntrp_rating FROM players").all<{ id: string; ntrp_rating: number }>()
  ).results;

  await db.batch(
    players.map((p: { id: string; ntrp_rating: number }) =>
  ...
  for (let i = 0; i < eloInserts.length; i += 20) {
    ...
    await db.batch(
      batch.map((e: typeof eloInserts[number]) =>
        db.prepare(
  ...
  const updateBatch = players.map((p: { id: string }) =>
    db.prepare("UPDATE players SET singles_elo = ?, doubles_elo = ? WHERE id = ?")
  ...
  return {
    tournamentMatches: tourneyMatches.length,
    leagueResults: leagueProcessed,
